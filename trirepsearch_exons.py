...
def rep_search(bioc_mol_type,
    ...
    def exon_length(gene_name):

        full_exon_length = 0 ; exon_number = [0]

        for row in gene_dict.to_dict('records'):

            if row['Gene'] == gene_name:

                if int(row['Exon']) not in exon_number:

                    exon_number += [int(row['Exon'])]
                    full_exon_length += int(row['Exon_Length'])

        return full_exon_length
    ...
    def full_coords(tscript_id, sense):
        ...
        exons_w_rep_coords = [] ; rep_coords = [] ; exon_number = 0
        ...
        for row in gene_dict.to_dict('records'):
            ...
            if row['Transcript'] == tscript_id:
                ...
                if int(row['Exon']) != exon_number:

                    exon_number = int(row['Exon'])

                    exons_w_rep_coords += list(range(row['Exon_Start'],
                                                     row['Exon_End'] + 1))
        ...
        return [coding_coords, exons_w_rep_coords, rep_coords,
                intron_coords, UTR_5_coords, UTR_3_coords,
                Flank_U_coords, Flank_D_coords]



    def exon_stitch(gene_ref, gene_coord, gene_dict, utr_len):

        stop = 0 ; rep_st = 0 ; rep_ed = 0

        for exon_ref in Exon_library:

            if exon_ref['Transcript_ID'] == gene_ref['Transcript_ID']:

                stop = 0 ; rep_st = 0 ; rep_ed = 0

                if int(exon_ref['Sense']) == 1:

                    if gene_coord[0] > int(exon_ref['Pos_End']) or \
                       gene_coord[1] < int(exon_ref['Pos_Start']):

                        stop = 1

                    rep_st = int(exon_ref['Exon_Start'])
                    rep_ed = int(exon_ref['Exon_End'])

                    if gene_coord[0] > int(exon_ref['Pos_Start']):

                        rep_st = (int(exon_ref['Exon_Start'])
                            + (gene_coord[0]
                            - int(exon_ref['Pos_Start']))
                            + 1)

                    if gene_coord[1] < int(exon_ref['Pos_End']):

                        rep_ed = (int(exon_ref['Exon_End'])
                            - (int(exon_ref['Pos_End'])
                            - gene_coord[1]))
                ...
                if stop != 1:
                    ...
                    DNAseq = gene_ref['Sequence']
                    AAseq = str(Seq(DNAseq).translate())

                    DNAseq_rep = gene_ref['Sequence'][
                                    gene_coord[0] : gene_coord[1]]

                    AAseq_rep = str(Seq(DNAseq_rep).translate())
                    Coding_Length = len(gene_ref['Sequence'])
                    Full_UTR_Length = utr_len[0] + utr_len[1]

                    Exon_Length = (int(exon_ref['Exon_End'])
                                 - int(exon_ref['Exon_Start'])) + 1

                    gene_dict = gene_dict.append({
                        'Gene': gene_ref['Gene'],
                        'Transcript': exon_ref['Transcript_ID'],
                        'CHR': exon_ref['CHR'],
                        'Sense': int(exon_ref['Sense']),
                        'Flank_Up_Stream' : 0,
                        'Flank_Down_Stream' : 0,
                        'Gene_Start_Up_Stream' : 0,
                        'Gene_End_Down_Stream' : 0,
                        'Exon': int(exon_ref['Exon_Num']),
                        'Exon_Length': Exon_Length,
                        'Exon_Start': int(exon_ref['Exon_Start']),
                        'Exon_End': int(exon_ref['Exon_End']),
                        'Repeat_Length' : len(list(range(rep_st,rep_ed))) + 1,
                        'Repeat_Start': rep_st,
                        'Repeat_End': rep_ed,
                        'Repeat_DNAseq': DNAseq_rep,
                        'Repeat_AAseq':  str(Seq(DNAseq_rep).translate()),
                        'Repeat_Amino_Acid_Count': len(AAseq_rep),
                        'Coding_DNAseq' : DNAseq,
                        'Coding_AAseq' : AAseq,
                        'Full_Amino_Acid_Count': len(AAseq) - 1,
                        'Full_Gene_Length' : 0,
                        'Coding_Length': len(DNAseq),
                        'Exons_W_Rep_Length' : 0,
    ...
    def domain_match(gene_name, coord_start, coord_end):
        ...
        CodingMut = 'No' ; ExonMut = 'No' ; RepMut = 'No'
        ...
        if any(x in coord_dict.at[gene_name,'coding_coords'] \
            ...
            if any(x in coord_dict.at[gene_name,'exons_w_rep_coords'] \
                   for x in [coord_start, coord_end]):
        ...
        return [CodingMut, ExonMut, RepMut, UTR5Mut, UTR3Mut, IntronMut,
    ...
    def mutation_effect(gene_name, allele_w, allele_m, coord):
        ...
        BlosumDict = blosum62 ; score = np.nan ; AA_M ='' ; AA_W =''
        ...
        if (AA_W, AA_M) in BlosumDict:

            score = int(BlosumDict[(AA_W, AA_M)])

        elif (AA_M, AA_W) in BlosumDict:

            score = int(BlosumDict[(AA_M, AA_W)])
    ...
    if os.path.isfile(os.path.join(pickle_path + 'gene_dict/',
        ...
        gene_dict = pd.DataFrame(columns = [
            ...
            'Repeat_DNAseq',
            'Repeat_AAseq',
            'Repeat_Amino_Acid_Count',
            'Coding_DNAseq',
            'Coding_AAseq',
            'Full_Amino_Acid_Count',
            'Full_Gene_Length',
            'Coding_Length',
            'Exons_W_Rep_Length',
            'Full_Repeat_Length',
        ...
        pbar = tqdm(total = CDS_library.shape[0])

        for gene in CDS_library.to_dict('records'):

            pbar.update(1)
            domain_len = 0 ; res_num = 0 ; gap_num = 0

            for codon_pos in range(round(len(gene['Sequence']) / 3)):

                domain_len += rep_nucs ; rep_len += rep_nucs

                if gene['Sequence'][(domain_len - 3):domain_len] in reps:
        ...
        for gene in gene_dict.to_dict('records'):
            ...
            gene_dict.at[i, 'Exons_W_Rep_Length'] = exon_length(gene['Gene'])
    ...
    for ID in range(len(gene_ID)):
        ...
        if os.path.isfile(os.path.join(pickle_path + 'ICGC_data_rep/',
            ...
            if no_muts == False:
                ...
                pbar = tqdm(total = ICGC_df.shape[0]) ; i = 0
                ...
                for row in ICGC_df.to_dict('records'):
                    ...
                    if ' of <=200bp' in row['type']:

                        ICGC_df.at[i,'type'] = ICGC_df.at[i,'type'][:-11]
                        ...
                        if ref_size == mut_size:
                            ...
                            ICGC_df.at[i,'type'] = 'multiple base substitution'
                    ...
                    if mut_location[0] == 'Yes':
                        ...
                        if 'single base substitution' in row['type']:
                            ...
                            mut_location[-1] = mut_type[0]
                            ICGC_df.at[i,'type'] += mut_type[1]

                        elif 'deletion' in row['type']:

                            if len(row['mutation'][:-2]) % 3 != 0:

                                ICGC_df.at[i,'type'] = 'deletion (frame shift)'

                            else:

                                ICGC_df.at[i,'type'] = 'deletion (in frame)'

                        elif 'insertion' in row['type']:

                            if len(row['mutation'][2:]) % 3 != 0:

                                ICGC_df.at[i,'type'] = 'insertion (frame shift)'

                            else:

                                ICGC_df.at[i,'type'] = 'insertion (in frame)'

                        elif 'multiple base substitution' in row['type']:

                            if abs(ref_size - mut_size) % 3 != 0:

                                ICGC_df.at[i,'type'] += ' (frame shift)'

                            else:

                                ICGC_df.at[i,'type'] += ' (in frame)'

                    else:

                        ICGC_df.at[i,'type'] += ' (non coding)'

                    Mut_df.loc[i] = mut_location ; i += 1
    ...
    if os.path.isfile(os.path.join(pickle_path + 'mut_type/',
            'Sub_Type_all' + search_param + '.pickle')) and os.path.isfile(
        ...
        os.path.join(pickle_path + 'mut_type/',
            'Mut_Type_general_all' + search_param + '.pickle')) and os.path.isfile(
        os.path.join(pickle_path + 'blosum_score/',
            'blosum_score' + search_param + '.pickle')):

        print('Unpickling blosum score data')

        blosum_df = pickle.load(open(os.path.join(pickle_path + 'blosum_score/',
            'blosum_score' + search_param + '.pickle'), "rb"))
        ...
        print('Unpickling substitution mutation data (all)')

        sub_df = pickle.load(open(os.path.join(pickle_path + 'mut_type/',
            'Sub_Type_all' + search_param + '.pickle'), "rb"))
        ...
        coding_regions = ['Coding', 'Exons w/ repeat', 'Repeat domain']

        domain_names = ['Coding', 'Exons w/ repeat', 'Repeat domain', "5'UTR",
                        "3'UTR", 'All Introns','All UTRs', 'Transcript',
                        'Up flank', 'Down flank', 'Intergene', 'Non coding']

        mut_names = ['coding_mut', 'exon_mut', 'rep_mut', 'UTR5_mut',
                     'UTR3_mut', 'intron_mut', 'UTR_mut', 'transcript_mut',
        ...
        sub_general = ['single base substitution', 'silent', 'missense',
                       'stop gained', 'stop lost', 'start lost']
        
        domain_num = len(domain_names) ; DomainLen_full = []
        blosum_df = pd.DataFrame(index = gene_list, columns = coding_regions)
        ...
        sub_df = pd.DataFrame(0, index = gene_list,
                                columns = (pd.MultiIndex.from_product(
                                    [sub_general, domain_names])))
        ...
        for gene in gene_dict.to_dict('records'):
            ...
            if gene['Gene'] != last:
                ...
                INFTotal = np.zeros(domain_num)
                FRSTotal = np.zeros(domain_num)
                SILTotal = np.zeros(domain_num)
                MISTotal = np.zeros(domain_num)
                STGTotal = np.zeros(domain_num)
                STLTotal = np.zeros(domain_num)
                STALTotal = np.zeros(domain_num)
                B62Total = np.zeros(3) ; B62Score = np.zeros(3)

                DomainLen_gene = [gene['Coding_Length'],
                                  gene['Exons_W_Rep_Length'],
            ...
            for mut in mut_dict_ref_gene.values:
                ...
                for coding in range(3):

                    if mut[8 + coding] == 'Yes':

                        if isinstance(mut[-1], int):

                            B62Score[coding] += mut[-1] ; B62Total[coding] += 1
                ...
                for domain in range(domain_num):
                    ...
                    if mut[8 + domain] == 'Yes':
                        ...
                        if 'single base substitution' in mut[1]:
                            ...
                            SBSTotal[domain] += 1 ; INFTotal[domain]  += 1
                            
                            if 'silent' in mut[1]:

                                SILTotal[domain] += 1
                            
                            elif 'missense' in mut[1]:
                                
                                MISTotal[domain] += 1
                                
                            elif 'stop gained' in mut[1]:
                                
                                STGTotal[domain] += 1
                                
                            elif 'stop lost' in mut[1]:
                                
                                STLTotal[domain] += 1
                                
                            elif 'start lost' in mut[1]:
                        ...
                        if 'in frame' in mut[1]:
                            
                            INFTotal[domain]  += 1
                            
                        if 'frame shift' in mut[1]:
                            
                            FRSTotal[domain]  += 1

            b62_values = np.zeros(3)
            gene_values = np.zeros(domain_num * 3)
            mut_values = np.zeros(domain_num * 6)
            sub_values = np.zeros(domain_num * 6)

            for coding in range(3):

                b62_values[coding] = B62Score[coding] / B62Total[coding]

            blosum_df.loc[gene['Gene']] = b62_values
            ...
            for domain in range(domain_num):
                ...
                if not MutTotal[domain] == 0:
                    ...
                    mut_values[(domain_num * 4) + domain] = INFTotal[domain]
                    mut_values[(domain_num * 5) + domain] = FRSTotal[domain]
                    
                    sub_values[domain] = SBSTotal[domain]
                    sub_values[domain_num + domain] = SILTotal[domain]
                    sub_values[(domain_num * 2) + domain] = MISTotal[domain]
                    sub_values[(domain_num * 3) + domain] = STGTotal[domain]
                    sub_values[(domain_num * 4) + domain] = STLTotal[domain]
                    sub_values[(domain_num * 5) + domain] = STALTotal[domain]

            heat_map_df.loc[gene['Gene']] = gene_values
            general_df.loc[gene['Gene']] = mut_values
            sub_df.loc[gene['Gene']] = sub_values
        ...
        blosum_df.loc['mean'] = blosum_df.mean()
        ...
        sub_df.loc['mean mut type count'] = sub_df.mean()

        sub_df.loc['mean domain length (BP)']  = mean_len * \
            int(len(sub_df.columns) / domain_num)

        sub_df.loc['mean mut type count per BP'] = \
            sub_df.loc['mean mut type count'] / \
            sub_df.loc['mean domain length (BP)']

        print('\nPickling blosum62 score data')

        pd.to_pickle(blosum_df, os.path.join(pickle_path + 'blosum_score/',
            'blosum_score' + search_param + '.pickle'))
        ...
        print('Pickling substitution mutation type data')
        
        pd.to_pickle(sub_df, os.path.join(pickle_path + 'mut_type/',
            'Sub_Type_all' + search_param + '.pickle'))
    ...
    sub_df_colnames = list(sub_df.columns.levels[0])
    ...
    mut_type_coding_df = mut_type_df[coding_colnames]
    mut_type_general_non_coding_df = general_df[general_df_colnames]
    mut_type_general_coding_df = general_df[general_df_colnames]
    sub_type_non_coding_df = sub_df[sub_df_colnames]
    sub_type_coding_df = sub_df[sub_df_colnames]
    ...
    sub_type_non_coding_df = sub_type_non_coding_df.loc[:,pd.IndexSlice[:,[
        "5'UTR", "3'UTR", 'All Introns', 'All UTRs', 'Up flank', 'Down flank',
        'Intergene', 'Non coding']]]

    mut_type_coding_df = mut_type_coding_df.loc[:,pd.IndexSlice[:,[
        'Coding', 'Exons w/ repeat', 'Repeat domain']]]
    
    mut_type_general_coding_df = mut_type_general_coding_df.loc \
        [:,pd.IndexSlice[:,['Coding', 'Exons w/ repeat', 'Repeat domain']]]
        
    sub_type_coding_df = sub_type_coding_df.loc[:,pd.IndexSlice[:,[
        'Coding', 'Exons w/ repeat', 'Repeat domain']]]
    ...
    final_dfs_list = [gene_dict, mut_dict_ref, heat_map_df, frequency_df,
        ...
        cell_loc_df, org_loc_df, can_loc_df, GO_df, mut_type_coding_df,
        mut_type_non_coding_df, general_df, mut_type_general_coding_df,
        mut_type_general_non_coding_df, blosum_df, sub_type_non_coding_df, 
        sub_type_coding_df]
    ...
    final_dfs_names = ['gene_dict', 'mut_dict', 'mut_heat_map', 'frequency',
        ...
        'mut_type_general', 'mut_type_general_coding_df', 
        'mut_type_general_non_coding_df', 'blosum_score',
        'sub_type_non_coding_df' , 'sub_type_coding_df'] ; i = 0
    ...
    print('\nmean: ' + str(impact_score_df.iloc[:-1].mean()[2]))
    print('gene_count: ' + str(len(gene_list)))
    print('stdev: ' + str(impact_score_df.iloc[:-1].std()[2]))
